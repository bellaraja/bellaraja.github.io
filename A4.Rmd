...
```{r}
sf_pums_clean %>% filter(SERIALNO == "2019HU0000927")
  group_by(SERIALNO) %>%
    summarize_all(first)
```
...
Create a logit model in which leadrisk is the predicted variable, and BLD, TEN, MV, and PUMA are the predictors. Show a summary of the model, and detail one example of a predicted likelihood score (pick a random record, state the predictor variables, and state the predicted score). This should match the technique shown in Chapter 5.4. (minimum result, does not require accounting for replicate weights) 
